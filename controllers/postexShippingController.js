...
exports.getCities = async (req, res) => {
    try {
        const cities = await postexService.getOperationalCities(req.user._id);
...
exports.getPickupAddresses = async (req, res) => {
    try {
        const data = await postexService.getPickupAddresses(req.user._id);
...
exports.createPickupAddress = async (req, res) => {
    try {
        const data = await postexService.createPickupAddress(req.user._id, req.body);
        res.json({ success: true, data });
    } catch (err) {
...
exports.getOrderTypes = async (req, res) => {
    try {
        const data = await postexService.getOrderTypes(req.user._id);
        res.json({ success: true, data });
    } catch (err) {
        res.status(500).json({ success: false, message: err.message });
    }
...
exports.createShipment = async (req, res) => {
    try {
        ...
        const integration = await PostExIntegration.findOne({ ownerId: req.user._id });
        ...
        const response = await postexService.createOrder(req.user._id, payload);

        if (String(response.statusCode) !== '200') {
            const errMsg = response.statusMessage || 'PostEx rejected the booking';
            await ShipmentLog.create({ orderId: order._id, action: 'CREATE_ORDER_FAILED', endpoint: '/v3/create-order', responsePayload: response, errorMessage: errMsg, success: false }).catch(() => {});
        ...
        await ShipmentLog.create({ orderId: order._id, action: 'CREATE_ORDER', endpoint: '/v3/create-order', responsePayload: response, statusCode: 200, success: true }).catch(() => {});
...
exports.trackSingle = async (req, res) => {
    try {
        ...
        const response = await postexService.trackOrder(req.user._id, trackingNumber);

        // Update local shipment record
        const shipment = await PostExShipment.findOne({ ownerId: req.user._id, postexTrackingNumber: trackingNumber });
...
exports.cancelShipment = async (req, res) => {
    try {
        ...
        const response = await postexService.cancelOrder(req.user._id, trackingNumber);
...
exports.getPaymentStatus = async (req, res) => {
    try {
        ...
        const response = await postexService.getPaymentStatus(req.user._id, trackingNumber);
...
exports.generateLoadSheet = async (req, res) => {
    try {
        ...
        const owned = await PostExShipment.find({ ownerId: req.user._id, postexTrackingNumber: { $in: trackingNumbers } });
        ...
        const pdfBuffer = await postexService.generateLoadSheet(req.user._id, trackingNumbers, pickupAddress);
...
exports.getInvoice = async (req, res) => {
    try {
        ...
        const response = await postexService.getAirwayBillUrl(req.user._id, trackingNumbers);
        res.json({ success: true, data: response });
    } catch (err) {
        res.status(500).json({ success: false, message: err.message });
    }
...
exports.getAllOrders = async (req, res) => {
    try {
        ...
        const response = await postexService.listOrders(req.user._id, { statusId, fromDate, toDate });
        res.json({ success: true, data: response });
    } catch (err) {
        res.status(500).json({ success: false, message: err.message });
    }
...
exports.saveShipperAdvice = async (req, res) => {
    try {
        ...
        const response = await postexService.saveShipperAdvice(req.user._id, trackingNumber, statusId, remarks);
        res.json({ success: true, data: response });
    } catch (err) {
        res.status(500).json({ success: false, message: err.message });
    }
