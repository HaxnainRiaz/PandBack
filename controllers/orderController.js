    ...
    try {
        ...
        try {
            ...
            if (settings && settings.shipping) {
                shipping = settings.shipping.fee;
                if (settings.shipping.freeShippingEnabled) {
                    const amountThresholdMet = settings.shipping.freeShippingThreshold > 0 && itemsTotal >= settings.shipping.freeShippingThreshold;
                    const quantityThresholdMet = settings.shipping.freeShippingQuantityThreshold > 0 && totalQuantity >= settings.shipping.freeShippingQuantityThreshold;
        ...
        for (const item of calculatedItems) {
            ...
            try {
                ...
                socketUtil.getIO().emit('product:update', updatedProduct);
        ...
        try {
            ...
            socketUtil.getIO().emit('order:new', populatedOrder);
